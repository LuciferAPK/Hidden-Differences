package com.cyrus.hidden_differences.ui.customview
...
class FlowLayout @JvmOverloads constructor(
    ...
    private var lineChildIndex: ArrayList<List<Int>?>? = null
    private var lineHeightList: ArrayList<Int>? = null
    private var lineWidthList: ArrayList<Int>? = null
    private var currentLineWidth = 0
    private var currentLineHeight = 0
    private var maxWidth = 0
    private var currentLineChildIndex: ArrayList<Int>? = null

    // vertical
    private var availableHeight = 0
    private var rowChildIndex: ArrayList<List<Int>?>? = null
    private var rowWidthList: ArrayList<Int>? = null
    private var rowHeightList: ArrayList<Int>? = null
    private var currentRowWidth = 0
    private var currentRowHeight = 0
    private var maxHeight = 0
    private var currentRowChildIndex: ArrayList<Int>? = null

    // common
    private var weightChildList: ArrayList<Int>? = null
    ...
    private fun measureHorizontally(widthMeasureSpec: Int, heightMeasureSpec: Int) {
            ...
            if (lp.width == LayoutParams.MATCH_PARENT) {
                if (currentLineWidth + lp.leftMargin + lp.rightMargin + childHorizontalSpacing <= availableWidth) {
                    ...
                    currentLineHeight = max(
                        currentLineHeight.toDouble(),
                        (childHeightSize + lp.topMargin + lp.bottomMargin
                                + childVerticalSpacing).toDouble()
                    ).toInt()
        ...
        for (i in lineChildIndex!!.indices) {
            val currentLineIndexList = lineChildIndex!![i]
            currentLineHeight = lineHeightList!![i]
            val currentLineTotalWidth = lineWidthList!![i]
            currentLineWidth =
                when (gravity) {
                    GRAVITY_CENTER -> getPaddingLeft() + (availableWidth - currentLineTotalWidth) / 2
                    GRAVITY_RIGHT, GRAVITY_BOTTOM -> getPaddingLeft() + (availableWidth - currentLineTotalWidth)
                    else -> getPaddingLeft()
                }
        ...
        val measuredWidth =
            if (widthMode == MeasureSpec.UNSPECIFIED || widthMode == MeasureSpec.AT_MOST) maxWidth + getPaddingRight() + getPaddingLeft() else widthSize
        val measuredHeight =
    ...
    private fun endLine(extraSpacing: Int) {
        ...
        maxWidth = max(currentLineWidth.toDouble(), maxWidth.toDouble()).toInt()
    }
    ...
    private fun measureWeightChildHorizontal(
        weightChildList: ArrayList<Int>?, extraSpacing: Int,
        totalWeight: Float
    ) {
        while (weightChildList!!.size > 0) {
            ...
            if (childWidthSize + lp.leftMargin + lp.rightMargin != 0) {
                currentLineHeight = max(
                    currentLineHeight.toDouble(),
                    (childHeightSize + lp.topMargin + lp.bottomMargin).toDouble()
                )
                    .toInt()
            }
            weightChild.measure(
    ...
    private fun measureVertically(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        for (i in 0 until childCount) {
            ...
            if (lp.height == LayoutParams.MATCH_PARENT) {
                if (currentRowHeight + lp.topMargin + lp.height + lp.bottomMargin + childVerticalSpacing <= availableHeight) {
                    ...
                    currentRowWidth = max(
                        currentRowWidth.toDouble(), (childWidthSize + lp.leftMargin + lp.rightMargin
                                + childHorizontalSpacing).toDouble()
                    ).toInt()
                    endRow(0)
                    newRow()
                } else {
        ...
        // then set position for all child
        var totalWidth = getPaddingLeft()
        for (i in rowChildIndex!!.indices) {
            val currentRowIndexList = rowChildIndex!![i]
            currentRowWidth = rowWidthList!![i]
            val currentRowTotalHeight = rowHeightList!![i]
            currentRowHeight =
                when (gravity) {
        ...
        setMeasuredDimension(
            if (widthMode == MeasureSpec.UNSPECIFIED || widthMode == MeasureSpec.AT_MOST) totalWidth + getPaddingRight() else widthSize,
            if (heightMode == MeasureSpec.UNSPECIFIED || heightMode == MeasureSpec.AT_MOST) maxHeight + getPaddingLeft() + paddingBottom else heightSize
        )
    }
    ...
    private fun endRow(extraSpacing: Int) {
        ...
        maxHeight = max(currentRowHeight.toDouble(), maxHeight.toDouble()).toInt()
    }
    ...
    private fun measureWeightChildVertically(
        ...
        while (weightChildList!!.size > 0) {
            ...
            if (childHeightSize + lp.topMargin + lp.bottomMargin != 0) {
                currentRowWidth = max(
                    currentRowWidth.toDouble(),
                    (childWidthSize + lp.leftMargin + lp.rightMargin).toDouble()
                )
                    .toInt()
            }
            weightChild.measure(
